...
class AudioManager {
  ...
  static void playSound(AudioType audio, {double volume = 1.0, loop = false}) {
    if( loop ) {
      FlameAudio.loop(audio.assetName);
      return;
    }
    
    FlameAudio.play(audio.assetName, volume: volume);
  }

  static void playMusicLevel() async {
    await FlameAudio.bgm.play(AudioType.musicLevel.assetName, volume: 0.5);
  }
  ...
  static void playIntroMusic() async {
    await FlameAudio.bgm.play(AudioType.musicIntro.assetName, volume: 0.5);
  }
